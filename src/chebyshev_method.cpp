...
ChebyshevMethod::ChebyshevMethod(const Grid &grid, const double eps,
                                 const unsigned max_iter, const unsigned K)
    : grid(grid), eps(eps), max_iter(max_iter), K(K), tau(K),
      accuracy(eps + 1.0), n_iter(0) {
...
auto ChebyshevMethod::MaxDifference(
