...
double Task::U(const double x, const double y) { return std::exp(1 - x * x - y * y); }

double Task::F(const double x, const double y) {
  return -4 * (1 - x * x - y * y) * std::exp(1 - x * x - y * y);
}

std::function<bool(std::size_t, std::size_t)> Task::GetNodeValidPredicate(const std::size_t n,
                                                                          const std::size_t m) {
